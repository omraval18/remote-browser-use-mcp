...
- Claude Code subscription uses a stored OAuth token from `auth login claude-code --access-token ...`, `CLAUDE_CODE_OAUTH_TOKEN`, or `ANTHROPIC_AUTH_TOKEN`. Use `claude setup-token` to create a long-lived token.
