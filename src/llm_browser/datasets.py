...
def build_dataset_prompt(task: DatasetTask, headless: bool = True) -> str:
    ...
    return (
        ...
        "For research pages, `from browser_helpers import *` makes fetch_text(url) and fetch_readable_text(url) return "
        "sliceable strings; use fetch_text_result(...) or fetch_readable_text_result(...) only when you need response metadata. "
        "Use html_to_text(markup) when raw HTML would waste context. "
        ...
        "extract_store_locator_locations(url_or_interface, save_to='locations.json') returns a dict with locations/count/sample "
