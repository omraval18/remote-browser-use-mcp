...
| Codex-shaped sub-agents prevent parent context blowups. | Durable `agent_edges` and `agent_messages`; canonical `/root/...` paths; sanitized fork modes; wait/list/message/followup/close tools; spawned/follow-up children now execute as isolated provider sessions; tests cover no child transcript copying and compact parent completion events. | Done |
